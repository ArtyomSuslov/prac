...
solution(int fsm[], unsigned fsm_size)
{
    ...
    printf("int\n");
    printf("main(void) {\n");
    printf("    int current_state = 0;\n");
    printf("    char no_flag = 0, finish_flag = 0;\n");
    printf("    int number;\n");
    printf("    while (1) {\n");
    printf("        scanf(\"%%i\", &number);\n");
    printf("        switch (number) {\n");
    printf("            case 0: {\n");
    printf("                finish_flag = 1;\n");
    printf("                break;\n");
    printf("            }\n");
    for (int i = 1; i <= fsm_size; ++i) {
        if (fsm[3 * i - 2] == 0) {
            continue;
        }
    printf("            case %i: {\n", fsm[3 * i - 2]);
        for (int j = i; j <= fsm_size; ++j) {
            if (fsm[3 * j - 2] == fsm[3 * i - 2]) {
    printf("                if (current_state == %i) {\n", fsm[3 * j - 3]);
